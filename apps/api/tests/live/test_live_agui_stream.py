...
endpoint invented. Making the stream drive a *live* turn is T2.
...
from lunaris_api.live.session.agui.session_events import session_events
...
async def test_a_session_with_no_turns_still_opens_and_closes_its_message() -> None:
    ...
    ``session_events`` deliberately opens a message even when there is nothing in it: a client that
    ...
    # Act
    events = list(session_events(empty, thread_id="t", run_id="r"))

    # Assert — a message that opens and closes, carrying no content frame.
...
async def test_a_failure_after_the_stream_opens_arrives_as_a_frame(
    ...
    monkeypatch.setattr(
        sys.modules["lunaris_api.live.session.agui.router"], "session_events", _explode
    )

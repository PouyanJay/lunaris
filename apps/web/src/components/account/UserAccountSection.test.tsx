...
// The section reads its account off useAuth; drive it through a mocked hook so each test can set the
...
describe("UserAccountSection", () => {
  ...
  it("shows a designed notice, not a blank, when there's no session", () => {
    ...
    render(<UserAccountSection onGoHome={onGoHome} />);
  ...
  it("shows the identity: email-derived name + email", () => {
    render(<UserAccountSection onGoHome={vi.fn()} />);

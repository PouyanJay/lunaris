"""Gate C — the factual gate (V2-T2). Every numeric figure a scene's narration states must be
supported by a claim the scene cites; a framing-only scene may assert no figure and no comparison.
A contract that smuggles an unsupported figure fails clean — the spec's
``narration_claim_check_vs_sources`` gate. The gate diffs the CONTRACT against the packet (content
is static once planned), so it runs before any render compute is spent."""
...
def _scene(*, sources: list[str], narration: str) -> SceneContract:
...
def _contract(scene: SceneContract) -> SceneContracts:
...
def test_a_grounded_figure_present_in_a_cited_claim_passes() -> None:
    ...
    # Act / Assert — no smuggling: every narrated figure is in the cited claim.
...
def test_a_figure_an_order_of_magnitude_off_from_its_claim_fails() -> None:
    # Arrange — the claim says 8; the narration says 80. Set membership is exact: "8" != "80".
    ...
    with pytest.raises(FactualGateError) as caught:
        FactualGate().check(contract, packet)
    assert "80" in caught.value.unsupported
    assert "8" not in caught.value.unsupported


def test_a_smuggled_figure_absent_from_every_cited_claim_fails() -> None:
    # Arrange — the scene cites c1 (which says nothing about 99%) but narrates "99% faster".
    ...
    # Act / Assert — the unsupported figure is named; the gate fails clean before any render.
    with pytest.raises(FactualGateError) as caught:
        FactualGate().check(contract, packet)
    assert caught.value.scene_id == "S1_scene"
    assert "99" in caught.value.unsupported


def test_a_framing_only_scene_may_carry_no_figure() -> None:
    # Arrange — a framing scene that asserts nothing has no business stating a number.
    contract = _contract(
        _scene(
            sources=[FRAMING_ONLY_SENTINEL],
            narration="Sorting touches 42 industries.",
        )
    )
...
def test_a_framing_only_scene_may_carry_no_comparison() -> None:
    # Arrange — a comparison ("faster than") is an empirical claim, forbidden in a framing scene.
    contract = _contract(
        _scene(
            sources=[FRAMING_ONLY_SENTINEL],
            narration="Merge sort is faster than bubble sort.",
        )
    )
...
def test_a_scene_citing_an_id_absent_from_the_packet_fails() -> None:
    # Arrange — a contract whose scene cites c7, which the packet never lists (defensive: the
    # planner rejects this, but the gate must not trust a figure against a non-existent claim).
    packet = _packet(_claim("c1", "Merge sort runs in O(n log n) time."))
...
def test_a_figure_in_a_beat_narration_is_also_checked() -> None:
    # Arrange — the smuggled figure hides in a beat's narration, not the scene-level script.
    packet = _packet(_claim("c1", "Merge sort runs in O(n log n) time."))
    scene = SceneContract(
        id="S1_scene",
        archetype="process/flow",
    ...
    # Act / Assert — the gate reads beat narration too, so the hidden 73% is caught.
    with pytest.raises(FactualGateError) as caught:
        FactualGate().check(_contract(scene), packet)
    assert "73" in caught.value.unsupported

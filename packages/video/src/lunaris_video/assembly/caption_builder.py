...
def build_webvtt(contract: VideoContract, manifest: TimingManifest) -> str:
    ...
    accumulates ``anim_s`` across every scene's beats in order, so a cue's clock matches where its
    beat actually plays in the final MP4. Silent beats (empty narration) advance the cursor but emit
    no cue. A silent (estimate) manifest yields just the ``WEBVTT`` header — captions ship only with
    narration (plan principle 8), so the player adds the track only when there is something to show.
    """
    cues: list[str] = []
    cursor = 0.0

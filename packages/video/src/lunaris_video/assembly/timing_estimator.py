from lunaris_video.schemas import BeatTiming, SceneTiming, TimingManifest, VideoContract
...
def estimate_timing(contract: VideoContract) -> TimingManifest:
    """The WPM-estimated ``TimingManifest`` for a silent, voice-ready video (skill Stage 2.5).
    ...
    silent now but every duration the narration needs already exists — the measured path
    (``ISpeechSynthesizer``) returns the same shape with ``estimated=False`` and clip paths.
    """
    ...
    for scene in contract.scenes:
        ...
        for beat in scene.beats:
            ...
            beats.append(
                BeatTiming(id=beat.id, audio_s=audio_s, anim_s=anim_s, audio=None, estimated=True)
            )
            total += anim_s

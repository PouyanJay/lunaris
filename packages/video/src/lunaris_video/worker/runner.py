...
async def run_video_workers(
    *,
    ...
    """Spawn ``count`` workers draining one shared queue (plus a lease sweep); run until cancelled
    or ``stop`` is set.
    ...
    and drains every task so a clean shutdown never orphans one. ``count`` is floored at 1 (a
    misconfigured 0 would stall the queue forever).
    ...
    Alongside the workers runs ONE lease sweep (V7-T4): every ``sweep_interval_seconds`` it requeues
    jobs a dead worker left in-flight past ``lease_seconds`` (a live render heartbeats, so its lease
    stays fresh) and dead-letters those past ``lease_max_attempts``. Running it in every replica is
    safe (the sweep is atomic + idempotent); the KEDA scaler counts stale in-flight rows too, so a
    job stuck after a scale-to-zero still wakes a replica that sweeps and re-claims it.
    """
    workers = [
        VideoWorker(
            queue=queue,
    ...
    _logger.info(
        "video_workers_started", count=len(worker_tasks), poll_seconds=poll_interval_seconds
    )
    try:
        if stop is None:
...
async def _sweep_leases_forever(
    queue: IVideoJobQueue, *, lease_seconds: int, max_attempts: int, interval_seconds: float
) -> None:
    """Sweep stale leases now and every ``interval_seconds`` after, until cancelled. Sweep-then-
    sleep so a freshly-woken replica recovers stuck jobs immediately. A sweep failure is logged,
    not fatal — the loop survives a transient queue blip."""
    while True:
        try:
            ...
            if result.requeued or result.dead_lettered:
                _logger.info(
                    "video_worker.lease_sweep",
                    requeued=result.requeued,
                    dead_lettered=result.dead_lettered,
                )
        except PersistenceError:
            _logger.warning("video_worker.lease_sweep_failed", exc_info=True)

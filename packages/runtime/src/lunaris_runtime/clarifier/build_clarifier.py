...
:class:`Clarification` by question id (level/knowledge/background/detail/language). They are folded
onto the brief by ``apply_clarification``. The enum questions are closed (the bands are exhaustive);
...
def build_clarifier(brief: CourseBrief) -> Clarifier:
    """Build the five confirm questions for ``brief``, each pre-picking the interpreter's guess."""
    return Clarifier(
        questions=[
            _choice(

...
class ICourseStore(Protocol):
    ...
    def load(self, course_id: str) -> Course:
        """Return the stored course, or raise ``FileNotFoundError`` when no course has that id — the
        store-agnostic not-found signal the caller catches to answer 404."""
        ...

    def delete(self, course_id: str) -> bool:
        """Delete the stored course. Idempotent: returns ``True`` if a row/file was removed,
        ``False`` if it was already absent — so the caller can choose 204 vs 404."""
        ...

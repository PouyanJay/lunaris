...
class ICostEventStore(Protocol):
    """Append-only persistence for a course's build-cost ledger — the drill-through source.
    ...
    ``delete_for_course`` when the whole course is purged.

    ``list_for_course`` feeds the ledger drill-through (``GET /api/courses/{id}/cost/events``) and
    the rollup recompute; it aggregates every run's events for the course. ``owner_id`` (per-user
    scoping) is the authenticated caller's id: ``append`` stamps it on each row, and
    ``list_for_course`` / ``delete_for_course`` constrain to it (another user's ledger reads as
